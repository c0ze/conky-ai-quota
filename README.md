...
Claude (Claude Code), Codex (OpenAI) and Grok (xAI) — with time-to-reset
next to each bar.
...
Each bar is weekly-plan utilization (Grok: billing period), the text is time
...
| `auth!` / `error` | no data yet: login rejected / other failure |
...
thread and does the network work (at most once per 240 s per cache, with the
same back-off after an error). `color`, `days` and `percent` only read the
